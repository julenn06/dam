# Sistemas Informáticos - DAM2

Esta carpeta contiene el módulo de **Sistemas Informáticos** del segundo curso de DAM, centrado en administración de sistemas, redes y virtualización.

## Contenido Principal

### 1.Eval/
- **Erronka SI.docx**: Proyecto erronka de Sistemas Informáticos.
- **Fernandez_Julen.pdf**: Documento personal.
- **RepasoExamenSI/**: Repaso de exámenes.

### 2. Eval/
- **Ariketa01/**: Ejercicio 1.

## Tecnologías
- **Sistemas Operativos**: Windows Server, Linux, scripts (Bash, PowerShell).
- **Redes**: Configuración TCP/IP, protocolos, seguridad, VPN.
- **Virtualización**: VMware, VirtualBox, Docker.
- **Hardware**: Componentes, mantenimiento, diagnóstico.
- **Seguridad**: Antivirus, firewalls, encriptación, auditoría.

## Objetivos
- Administrar sistemas operativos y redes.
- Implementar virtualización y cloud.
- Gestionar hardware y seguridad informática.
- Automatizar tareas con scripts.

Este módulo cubre la infraestructura de sistemas.